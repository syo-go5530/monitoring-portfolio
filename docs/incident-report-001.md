...
| 時刻 | 出来事 |
|---|---|
| 06:02 | 06:02 Nginx コンテナを停止（障害発生） |
| （+15秒ほど） | 06:02 probe_success が 1 → 0 に変化（監視が異常を検知） |
| （+1分） | 06:03 WebServiceDown アラートが PENDING → FIRING に遷移、Alertmanager へ通知 |
| 06:05 | 06:05 Nginx コンテナを再起動（復旧作業） |
| （+15秒ほど） | 06:05 probe_success が 0 → 1 に回復、アラートが Inactive へ戻る |
